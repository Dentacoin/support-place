	...
	<div class="categories-wrapper">
		<div class="flex space-between container">
			@foreach($categories as $cat)
				<a href="javascript:;" class="category {{ !empty($current_question) ? ($current_question->category_id == $cat->id ? 'active' : '') : ($loop->first ? 'active' : '') }}" cat-id="{{ $cat->id }}">{{ $cat->name }}</a>
			@endforeach
		</div>
	</div>
	<div class="categories-questions container">
		@foreach($categories as $cat)
			<div class="category-questions {{ !empty($current_question) ? ($current_question->category_id == $cat->id ? 'active' : '') : ($loop->first ? 'active' : '') }}" id="cat-{{ $cat->id }}">
